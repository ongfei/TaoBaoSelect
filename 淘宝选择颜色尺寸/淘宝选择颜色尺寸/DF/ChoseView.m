...
@synthesize alphaiView,whiteView,img,lb_detail,lb_price,lb_stock,mainscrollview,sizeView,colorView,countView,bt_sureAdd,bt_sureBuy,bt_cancle,lb_line;
   ...
   self = [super initWithFrame:frame];
    if (self) {
       ...
       //装载商品信息的视图
        ...
        //确定按钮add
        bt_sureAdd = [UIButton buttonWithType:UIButtonTypeCustom];
        bt_sureAdd.frame = CGRectMake(0, whiteView.frame.size.height-50,whiteView.frame.size.width/2 - 1, 50);
        [bt_sureAdd setBackgroundColor:[UIColor redColor]];
        [bt_sureAdd setTitleColor:[UIColor whiteColor] forState:0];
        bt_sureAdd.titleLabel.font = [UIFont systemFontOfSize:20];
        [bt_sureAdd setTitle:@"加入购物车" forState:0];
        [whiteView addSubview:bt_sureAdd];
        
        //确定按钮add
        bt_sureBuy = [UIButton buttonWithType:UIButtonTypeCustom];
        bt_sureBuy.frame = CGRectMake(CGRectGetMaxX(bt_sureAdd.frame) + 1, whiteView.frame.size.height-50,whiteView.frame.size.width/2, 50);
        [bt_sureBuy setBackgroundColor:[UIColor redColor]];
        [bt_sureBuy setTitleColor:[UIColor whiteColor] forState:0];
        bt_sureBuy.titleLabel.font = [UIFont systemFontOfSize:20];
        [bt_sureBuy setTitle:@"立即购买" forState:0];
        [whiteView addSubview:bt_sureBuy];

        //有的商品尺码和颜色分类特别多 所以用UIScrollView 分类过多显示不全的时候可滑动查看
        mainscrollview = [[UIScrollView alloc] initWithFrame:CGRectMake(0, lb_line.frame.origin.y+lb_line.frame.size.height, whiteView.frame.size.width, bt_sureAdd.frame.origin.y-(lb_line.frame.origin.y+lb_line.frame.size.height))];
